...
1227 D2 - Optimal Subsequences (Hard Version).cpp GNU C++14 Accepted
...
int main(){
	...
	sort(all(vec));
	cin>>m;
	vector<pair<pll, ll>> qrs(m);
	rep(i,0,m-1){
		cin>>qrs[i].ff.ff>>qrs[i].ff.ss;
		qrs[i].ss = i;
	}
	sort(all(qrs));
	vector<ll> ans(m);
	...
	int l=0;
	rep(i,0,m-1){
		// cout<<qrs[i].ff.ss<<" = "<<qrs[i].ss<<endl;
		while(l<qrs[i].ff.ff) sam.in(vec[l++].ss);
		ans[qrs[i].ss] = a[*sam.find_by_order(qrs[i].ff.ss-1)];
	}
