...
1228 D - Complete Tripartite.cpp GNU C++14 Accepted
...
int main(){
	...
	rep(i,0,2){
		int first=1;
		while(grp[first]!=-1) first++;
		if(first==n+1){cout<<"-1\n"; return 0;}
		grp[first] = i;
		rep(second, 1, n){
			if(first!=second && grp[first]!=grp[second] && edges[first].find(second)==edges[first].end()) grp[second]=i;
		}
	}	
	vector<int> groups[3];
	rep(i,1,n){
		if(grp[i]==-1){cout<<"-1\n";return 0;}
		else groups[grp[i]].pb(i);
	}

	int tot=0;
	rep(i,0,2){
		rep(j,i+1,2){
			for(auto it1:groups[i]){
				for(auto it2:groups[j]){
					if(edges[it1].find(it2)==edges[it1].end()){cout<<"-1\n"; return 0;}
					else tot++;
				}
			}
		}
