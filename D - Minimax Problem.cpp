...
1288 D - Minimax Problem.cpp GNU C++14 Accepted
...
int main(){
	...
	while(l<=r){
		...
		rep(i,0,n-1){
			rep(j,0,m-1){
				if(mat[i][j] >= mid) bits[i] += (1<<j);
			}
		}
		sort(bits, bits+n);
		rep(i,0,n-1){
			rep(j,0,bit){
				if((bits[i]|j)==bit && binary_search(bits, bits+n, j)){
					flag = true;
					i1 = bits[i];
					i2 = j;
					break;
	...
	rep(i,0,n-1){
		rep(j,0,m-1){
			if(mat[i][j] >= ans) bits[i] += (1<<j);
		}
	}
	rep(i,0,n-1){
		if(bits[i]==i1) {cout<<i+1<<" ";break;}
	}
	rep(i,0,n-1){
		if(bits[i]==i2) {cout<<i+1<<" ";break;}
	}
}
