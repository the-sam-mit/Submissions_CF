...
1285 D - Dr. Evil Underscores.cpp GNU C++14 Accepted
...
ll func(vector<ll> &v, int ind){
    if(ind<0) return 0;
    vector<ll>st,unst;
    rep(i,0,sz(v)-1){
        if((v[i]>>ind)&1) st.pb(v[i]);
        else unst.pb(v[i]);
    }
    if(sz(st)==0) return func(unst, ind-1);
    if(sz(unst)==0) return func(st, ind-1);
    return (1<<ind)+min(func(st, ind-1), func(unst, ind-1));
}
