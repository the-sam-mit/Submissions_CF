...
1223 D - Sequence Sorting.cpp GNU C++14 Accepted
...
int main(){
	...
	testcase(T){
		...
		int a[n];
		vector<int> v;
		int mnpos[n+1], mxpos[n+1], dp[n+1];
		rep(i,0,n){
			mnpos[i]=n+1;
			mxpos[i]=-1;
			dp[i]=0;
		...
		rep(i,0,n-1){
			...
			a[i]--;
			v.pb(a[i]);
			mnpos[a[i]] = min(mnpos[a[i]], i);
			mxpos[a[i]] = max(mxpos[a[i]], i);
		}	
		sort(all(v));
		v.resize(distance(v.begin(), unique(all(v))));
		int ans = n;
		nrep(i,0,sz(v)-1){
			if(i+1==sz(v) || mxpos[v[i]]>mnpos[v[i+1]]) dp[i] = 1;
			else dp[i] = dp[i+1] + 1;
