...
int main(){
    ...
    rep(i,0,q-1){
        ...
        if(x==1){
            m1[x+y] = (m1[x+y]+1)%2;
            if(m1[x+y]%2==0){
                if(mps.find(mp(sum, sum))!=mps.end()) mps.erase(mp(sum, sum));
                if(mps.find(mp(sum, sum+1))!=mps.end()) mps.erase(mp(sum, sum+1));
                if(mps.find(mp(sum, sum+2))!=mps.end()) mps.erase(mp(sum, sum+2));
            }
            else{
                if(m2[sum]%2) mps.in(mp(sum,sum));
                if(m2[sum+2]%2) mps.in(mp(sum,sum+2));
                if(m2[sum+1]%2) mps.in(mp(sum,sum+1));
            }
        }
        else{
            m2[x+y] = (m2[x+y]+1)%2;
            if(m2[x+y]%2==0){
                if(mps.find(mp(sum-1, sum))!=mps.end()) mps.erase(mp(sum-1, sum));
                if(mps.find(mp(sum-2, sum))!=mps.end()) mps.erase(mp(sum-2, sum));
                if(mps.find(mp(sum, sum))!=mps.end()) mps.erase(mp(sum, sum));
            }
            else{
                if(m1[sum-1]%2) mps.in(mp(sum-1,sum));
                if(m1[sum-2]%2) mps.in(mp(sum-2,sum));
                if(m1[sum]%2) mps.in(mp(sum,sum));
            }
        }
        if(sz(mps)>0){
            cout<<"No\n";
