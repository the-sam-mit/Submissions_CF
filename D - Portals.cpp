...
1271 D - Portals.cpp GNU C++14 Accepted
...
int main(){
	...
	rep(i,0,n-1){
		last[i]=i;
		for(auto it:adj[i]) last[i]=max(last[i], it);
		insertV[last[i]].pb(c[i]);	
	}
	rep(i,0,n-1){
		if(k<a[i]){
			cout<<"-1\n";
		...
		while(checkd.size()>(k-a[i])) checkd.erase(checkd.begin());
		k+=b[i];
		for(auto it:insertV[i]) checkd.insert(it);
	}
	while(checkd.size()>k) checkd.erase(checkd.begin());
	ll ans=0;
	for(auto it:checkd) ans+=it;
	cout<<ans<<endl;
}